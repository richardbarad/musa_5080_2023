...
```{r price_map, fig.height=4, fig.width=4}
# ggplot, reorder
...
```{r crime_corr, fig.width=8, fig.height=6}
## Crime cor
boston.sf %>%
  st_drop_geometry() %>%
  mutate(Age = 2015 - YR_BUILT) %>%
...
reg1 <- lm(SalePrice ~ ., data = boston_sub_200k %>% 
                                 dplyr::select(SalePrice, LivingArea, Style, 
                                               ...
                                               R_BDRMS, R_FULL_BTH,
                                               R_KITCH, R_AC, R_FPLACE, crime_nn2))
...
```{r}
reg <- lm(SalePrice ~ ., data = boston_sub_200k %>%
            dplyr::select(GROSS_AREA,R_BDRMS,R_AC,crime_nn2,NUM_FLOORS,SalePrice))
